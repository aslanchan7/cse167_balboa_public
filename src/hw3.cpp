...
void hw_3_3(const std::vector<std::string> &params)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int n = 0; n < numMeshes; n++)
        {
            ...
            // set light color
            shader.setVec3("lightColor", 1.0f, 0.5f, 0.5f);
...
void hw_3_4(const std::vector<std::string> &params)
{
    ...
    // VBO, VAO & EBO
    int numMeshes = scene.meshes.size();
    unsigned int VAO[numMeshes], VBO_vertex[numMeshes], VBO_color[numMeshes], EBO[numMeshes], VBO_normal[numMeshes];

    glGenVertexArrays(numMeshes, VAO);
    glGenBuffers(numMeshes, EBO);
    glGenBuffers(numMeshes, VBO_vertex);
    glGenBuffers(numMeshes, VBO_normal);
    glGenBuffers(numMeshes, VBO_color);
    ...
    for (int i = 0; i < numMeshes; i++)
    {
        ...
        int normalSize = 3 * scene.meshes[i].vertex_normals.size();
        float normals[normalSize];
        for (int j = 0; j < (int)scene.meshes[i].vertex_normals.size(); j++)
        {
            normals[(3 * j) + 0] = scene.meshes[i].vertex_normals[j].x;
            normals[(3 * j) + 1] = scene.meshes[i].vertex_normals[j].y;
            normals[(3 * j) + 2] = scene.meshes[i].vertex_normals[j].z;
        }
    ...
    // shader class
    Shader shader = Shader("C:/Users/Aslan Chan/Classes/2025-2026/CSE 167/cse167_balboa_public/src/shaders/shader.vs", "C:/Users/Aslan Chan/Classes/2025-2026/CSE 167/cse167_balboa_public/src/shaders/shader.fs");
    // Shader lightShader = Shader("C:/Users/Aslan Chan/Classes/2025-2026/CSE 167/cse167_balboa_public/src/shaders/lightSourceShader.vs", "C:/Users/Aslan Chan/Classes/2025-2026/CSE 167/cse167_balboa_public/src/shaders/lightSourceShader.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // glm::mat4 view;
        // view = glm::lookAt(glm::vec3(camX, cameraPos.y, camZ), cameraPos + cameraFront, cameraUp);
        ...
        for (int n = 0; n < numMeshes; n++)
        {
            // create model matrix & normal matrix
            glm::mat4 model = glm::mat4(1.0f);
            Matrix4x4f modelMatrix = scene.meshes[n].model_matrix;
            for (int i = 0; i < 4; i++)
            {
